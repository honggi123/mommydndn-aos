...
internal fun _SignInScreen(
    viewModel: _SignInViewModel = hiltViewModel(),
    ...
    val naverCallback = object : OAuthLoginCallback {
        override fun onSuccess() {
            ...
            if (token != null) {
                viewModel.signIn(tokenId = token, type = OAuthProvider.NAVER, navHostController)
            }
        }
...
private fun getGoogleSignInClient(
    ...
    return GoogleSignInOptions.Builder(googleSignInOptions).requestEmail()
        .requestServerAuthCode(serverClientId).build()
        .let { options -> GoogleSignIn.getClient(context, options) }
}

private fun loginWithKakaoNickName(
    token: OAuthToken, viewModel: _SignInViewModel, navHostController: NavHostController
) {
    UserApiClient.instance.me { user, error ->
        when {
            ...
            user != null -> {
                viewModel.signIn(
                    tokenId = token.accessToken, type = OAuthProvider.KAKAO, navHostController
                )
            }
        }
    }
