...
internal fun CareJobOpeningListFragment(
    careJobOpenings: List<CareJobOpening>,
    modifier: Modifier = Modifier,
) {
    Column(modifier = modifier.background(Color.White)) {
        careJobOpenings.forEachIndexed { index, jobOpening ->
            with(jobOpening) {
                ...
                if (index != careJobOpenings.lastIndex) {
...
private fun PreviewCareJobOpeningListFragment() {
    Box(modifier = Modifier.fillMaxSize().background(Color.White)) {
        CareJobOpeningListFragment(
            careJobOpenings = buildList(capacity = 3) {
                add(
                    CareJobOpening(
                        workPeriod = WorkPeriod.REGULAR,
                        careTypes = listOf(
